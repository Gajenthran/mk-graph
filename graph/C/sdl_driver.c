...
static void init_graph_coord(void);
static void draw_circle(SDL_Point center, int radius, SDL_Color color);
static void draw_title(void);
static void draw_vertices(void);
static void draw_paths(fstack_t ** paths);
static void draw_edge(fstack_t ** paths);
...
#define GRAPH (sdl_driver.graph)
#define IN    (sdl_driver.in)
...
static int init(const graph_t* graph) {
  GRAPH = graph;
  sdl_driver.in = malloc(sizeof *sdl_driver.in);
  assert(sdl_driver.in);
  sdl_driver.in->quit = 0;
  sdl_driver.in->paths = -1;
  init_SDL();
  visited_cir = (int *)calloc(0, GRAPH->vec.nbn * sizeof(*visited_cir));
  assert(visited_cir);
  return 0;
}
/** 
...
int init_SDL(void) {
  ...
  win = SDL_CreateWindow("PCC: Dijkstra", 0, 0, 1280, 760, SDL_WINDOW_SHOWN | SDL_WINDOW_FULLSCREEN);
  if(win == NULL) {
...
int choose_paths(fstack_t ** paths) {
  if(GRAPH->model == LIS_E) {
    clean_tab(visited_cir, GRAPH->vec.nbn);
  } else {
    clean_tab(visited_cir, GRAPH->mat.nbn);    
  }

  if(sdl_driver.in->paths != -1) {
    int i;
    while(!empty_stack(paths[sdl_driver.in->paths])) {
      i = pop_stack(paths[sdl_driver.in->paths]);
      visited_cir[i] = 1;
    }
    rewind_stack(paths[sdl_driver.in->paths]);
    return 1;
  }
  return 0;
}
 ...
 */
static int load_texts(void) {
  textTex = (SDL_Texture **)malloc((GRAPH->vec.nbn + TEXTS_E) * sizeof(*textTex));
  assert(textTex);

  ntexts = GRAPH->vec.nbn + TEXTS_E;
  int i;
  font = TTF_OpenFont("files/Roboto-Condensed.ttf", 30);
  SDL_Surface *t;
  for(i = 0; i < TEXTS_E + GRAPH->vec.nbn; ++i) {
    if(i < TEXTS_E) {
      t = TTF_RenderText_Solid(font, texts[i], black);
    } else {
      t = TTF_RenderText_Solid(font, strndup(GRAPH->vec.n[i - TEXTS_E].name, 5), black);
    }
    if(t == NULL) {
...
static void init_graph_coord(void) {
  srand(time(NULL));
  int xr, yr, i;
  graph_rect = (SDL_Rect *)malloc(GRAPH->vec.nbn * sizeof(*graph_rect));
  assert(graph_rect);
...
static void draw(fstack_t ** paths, int dep) {
  ...
  for(i = 0; i < ntexts - TEXTS_E; i++) {
    ...
    if(visited_cir[i]) draw_circle(p, 20, blue);
    else draw_circle(p, 20, red);
...
static void draw_paths(fstack_t ** paths) {
  ...
  for(i = 0; i < GRAPH->vec.nbn; i++) {
    sprintf(str, "%s: %d", strndup(GRAPH->vec.n[paths[i]->index].name, 5), paths[i]->dist);
    ...
    if(sdl_driver.in->paths == i) draw_circle(p, 5, red);
...
static void keydown(void) {
  ...
  while(SDL_PollEvent(&ev) != 0) {
    switch(ev.type) {
      case SDL_QUIT:
        sdl_driver.in->quit = 1;
        break;
      case SDL_MOUSEBUTTONDOWN:
        if(ev.button.button == SDL_BUTTON_LEFT) {
          sdl_driver.in->mbx = ev.button.x;
          sdl_driver.in->mby = ev.button.y;
        }
        break;
      default:
        break;
 ...
 */
static int element_clicked(int x0, int y0, int x1, int y1) {
  if(sdl_driver.in->mbx > x0 && sdl_driver.in->mby > y0 && sdl_driver.in->mbx < x1 && sdl_driver.in->mby < y1) {
    sdl_driver.in->mbx = sdl_driver.in->mby = 0;
    return 1;
  }
  return 0;
}
